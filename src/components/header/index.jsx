...
function Header() {
    ...
    const [data] = useGetData('/discover/movie?primary_release_date.gte=2014-09-15&primary_release_date.lte=2014-10-22')
    return (
        data && (
            <div className={classes['header']}>
                <img src={`https://image.tmdb.org/t/p/w1280/${data[0].backdrop_path}`} alt={data[0].original_title} className={classes["header__image"]} />
                <div className={classes["header__info"]}>
                    <h1 className={classes["header__title"]}>{data[0].original_title}</h1>
                    <p className={classes["header__text"]}>{data[0].overview}</p>
                    <button onClick={() => navigate(`/movie/${data[0].id}`)} className={classes['header__button']}>
