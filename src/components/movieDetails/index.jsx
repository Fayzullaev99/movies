  ...
  if (loading) {
    ...
    return (
      <div className={classNames(classes['details'])}>
        <img src={`${backdrop_path ? img_url+backdrop_path : defaultImg}`} alt={original_title} className={classes["details__image"]} />
        <div className={classes["details__block"]}>
          <div className={classes["details__info"]}>
            ...
            {genres.map((genre)=>(
              <span key={genre.id} className={classes["details__history"]}>{genre.name}</span>
            ))}
            ...
            <p className="details__actors-info">В главных ролях</p>
          </div>
